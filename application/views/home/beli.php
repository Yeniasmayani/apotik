...
<div class="container" style="margin-bottom: 50px;">
    <table class="table table-striped">
        ...
        <tbody>
            ...
            <tr id="item-<?= $item->kode_obat; ?>">
                ...
                <td><?= $item->nama; ?></td>
                <td><input type="number" value="<?= $item->jumlah; ?>" data-item="<?= $item->kode_obat; ?>"
                        class="jumlah" style="width: 50px;" onchange="$(this).updateJumlah()" min="1"></td>
                <td><?= number_format($item->harga, 0, ',', '.'); ?></td>
                <td id="subtotal-<?= $item->kode_obat; ?>"><?= number_format($item->subtotal, 0, ',', '.'); ?></td>
                <td><button class="btn btn-danger btn-xs" data-item="<?= $item->kode_obat; ?>"
                        onclick="$(this).delItem()"><span class="glyphicon glyphicon-remove"></span> Hapus</button></a>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
    ...
    <div class="col-md-6">
        ...
        <form action="" method="post" id="myForm">
            ...
            <p><strong>NB :</strong> Untuk melanjutkan pembelian wajib mengisi data
                dengan benar.</p>
            <input type="submit" class="btn btn-success" name="proses_beli" id="proses_beli" value="PESAN">
            <a href="<?= base_url('obat') ?>" class="btn btn-primary">Simpan & Kembali</a>
            <input type="submit" class="btn btn-warning" name="batal_beli" id="batal_beli" value="BATALKAN PEMESANAN">
        </form>
    </div>
</div>
